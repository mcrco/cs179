...
__global__ void residualAddKernel(__nv_bfloat16 *residual, value_t *values, int n) {
    ...
    for (int i = idx; i < n; i += stride) {
        ...
        residual[idx] = __float2bfloat16(rval + vval);
    }
}
...
void Qwen2Layer<QWEN2_SIZE>::forward(const std::shared_ptr<CudaBuffer>& k_cache, const std::shared_ptr<CudaBuffer> &v_cache, const std::shared_ptr<CudaBuffer> &hidden_state, int32_t seq_len, cudaStream_t stream) {
    ...
    // Query projection + RoPE (store inside state).
    ...
    RoPE::apply_rope_to_qk(queries_ptr, Qwen2Config::num_query_heads(), Qwen2Config::head_size(), seq_len, Qwen2Config::rope_theta_base(), stream);
    // Compute where current token's KV vectors should be stored in KV cache.
    ...
    // Key and value projections (store inside cache).
    ...
    // RoPE for keys.
    RoPE::apply_rope_to_qk(key_ptr, Qwen2Config::num_kv_heads(), Qwen2Config::head_size(), seq_len, Qwen2Config::rope_theta_base(), stream);

    // Self-attention.
    // Get pointer from cuda buffer.
    float *attention_output_ptr = static_cast<float *>(attention_output->data);
    // Store attention inside state.
