...
/*************** END GAUSSIAN ******************/
    ...
    // Allocate memory for arrays with padding.
    checkCuda(cudaMalloc(&dev_input_data, padded_length * sizeof(cufftComplex)));
    checkCuda(cudaMalloc(&dev_impulse_v, padded_length * sizeof(cufftComplex)));
    checkCuda(cudaMalloc(&dev_out_data, padded_length * sizeof(cufftComplex)));
...
#define STOP_RECORD_TIMER(name) {                           \
    ...
    for (int ch = 0; ch < nChannels; ch++){
        ...
        const unsigned int blocks = std::min( max_blocks,
            (unsigned int) ceil(padded_length/(float)local_size) );
        ...
        // Copy over input and impulse to device.
        checkCuda(cudaMemcpy(dev_input_data, input_data,
            N * sizeof(cufftComplex), cudaMemcpyHostToDevice));
        checkCuda(cudaMemcpy(dev_impulse_v, impulse_data,
        ...
        // Inverse FFT for output to get actual convolution results.
        ...
        checkCuda(cudaMalloc(&dev_max_abs_val, sizeof(float)));

        checkCuda(cudaMemset(dev_max_abs_val, 0, sizeof(float)));
        ...
        checkCuda(cudaMemcpy(output_data, dev_out_data, padded_length * sizeof(cufftComplex), cudaMemcpyDeviceToHost));
